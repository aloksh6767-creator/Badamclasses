...
export default function MockTestsPage() {
  ...
  useEffect(() => {
    ...
    const onlineMocks = Array.isArray(workspace.mockTests)
      ? workspace.mockTests.filter((item) => item?.status === "published")
      : [];
    setMockTests(onlineMocks.length ? onlineMocks : fallbackOnlineMocks);
  }, []);
  ...
  return (
    <main className="mx-auto w-[92%] max-w-6xl py-10 text-slate-100">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-4xl">Online Mock Tests</h1>
          <p className="mt-2 text-sm text-slate-300">
            Sirf online mock tests yahan show honge. Admin panel se publish kiye gaye mocks students ke liye yahin visible rahenge.
          </p>
        </div>
        <Link href="/" className="rounded-lg border border-white/20 px-4 py-2 text-sm">Back to Home</Link>
      </div>

      <div className="mb-6 rounded-2xl border border-cyan-300/20 bg-cyan-500/10 p-4 text-sm text-cyan-100">
        {publishedCount} online mock test{publishedCount === 1 ? "" : "s"} available. Offline registration form is page par show nahi hoga.
      </div>
