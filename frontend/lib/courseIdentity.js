...
export const normalizeCourseRouteValue = (value) => {
...
export const buildCourseRouteId = (course, index = 0) => {
  ...
  if (slug) {
    return `${slug}-${buildCourseFingerprint(course)}`;
  }

  return buildLegacyCourseRouteId(course, index);
};

export const buildCourseReactKey = (course, index = 0, scope = "course") => {
  return `${scope}:${buildCourseRouteId(course, index)}`;
};
